<!-- <input id="duration" data-suffix="Hour" value="50" min="0" max="100" type="number" />
 -->
<div class="form-group{{ $errors->has($id) ? ' has-danger' : '' }}  @isset($class) {{$class}} @endisset">
   ...
   @endif
   <input @isset($accept) accept="{{ $accept }}" @endisset data-suffix="Hour" min="1" max="8" type="{{ isset($type)?$type:"number"}}" name="{{ $id }}" id="{{ $id }}" class="form-control form-control-alternative @isset($editclass) {{$editclass}} @endisset  {{ $errors->has($id) ? ' is-invalid' : '' }}"  value="{{ old($id)?old($id):(isset($value)?$value:(app('request')->input($id)?app('request')->input($id):null)) }}" <?php if($required) {echo 'required';} ?> <?php if(isset($disabled) && $disabled) {echo 'disabled';} ?> autofocus>
