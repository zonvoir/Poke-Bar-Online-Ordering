    ...
    <div class="row">
        <div class="col-xl-6">
            ...
            <div class="card bg-secondary shadow">
                ...
                <form method="post" action="{{ route('restaurant.setting') }}" autocomplete="off">
                    @csrf
                    <input type="hidden" id="rid" name="rid" value="{{ $restorant->id }}"/>
                    <div class="pl-lg-4">
                        <div class="form-group{{ $errors->has('checkin_type') ? ' has-danger' : '' }}">
                            <label class="form-control-label" for="checkInPopup">{{ __('Check in type') }}</label>
                            <div class="custom-control form-check form-check-inline">
                              <input class="form-check-input" type="checkbox" name="checkin_type" id="checkInPopup" value="popup" {{$restorant->checkin_type == 'popup' ? 'checked' : ''}}>
                              <label class="form-check-label" for="checkInPopup">{{ __('Popup') }}</label>
                          </div>
                      </div>
                      <div class="form-group{{ $errors->has('allow_pickup') ? ' has-danger' : '' }}">
                        <label class="form-control-label" for="allow_pickup">{{ __('Pickup allowed ?') }}</label>
                        <select class="form-control form-control-alternative" id="allow_pickup" name="allow_pickup">
                            <option value="YES" {{$restorant->allow_pickup == 'YES' ? 'selected' : ''}}>Yes</option>
                            <option value="NO" {{$restorant->allow_pickup == 'NO' ? 'selected' : ''}}>No</option>
                        </select>
                    </div>
                      <div class="form-group{{ $errors->has('checkin_disclaimers') ? ' has-danger' : '' }}">
                        <label class="form-control-label" for="checkin_disclaimers">{{ __('Check in disclaimers') }}</label>
                        <textarea class="form-control form-control-alternative ckeditor" id="checkin_disclaimers" name="checkin_disclaimers">{{ old('checkin_disclaimers', $restorant->checkin_disclaimers) }}</textarea>
                    </div>
                    <div class="form-group{{ $errors->has('checkin_summery_disclaimers') ? ' has-danger' : '' }}">
                        <label class="form-control-label" for="checkin_summery_disclaimers">{{ __('Check in summery disclaimers') }}</label>
                        <textarea class="form-control form-control-alternative ckeditor" id="checkin_summery_disclaimers" name="checkin_summery_disclaimers">{{ old('checkin_summery_disclaimers', $restorant->checkin_summery_disclaimers) }}</textarea>
                    </div>
                </div>
                <div class="text-center">
                    <button type="submit" class="btn btn-success mt-4">{{ __('Save') }}</button>
