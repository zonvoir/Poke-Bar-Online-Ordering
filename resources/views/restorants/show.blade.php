  ...
  <div class="modal-dialog modal-dialog-scrollable cust_sco" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="checkInModalTitle">{{ __('Check In')}}</h5>
      ...
      <div class="modal-body">
        <form id="checkInModalForm">
          ...
          <div class="col-md-12">
           <p class="wr_dic_tec">{!! $restorant->checkin_disclaimers !!}</p>
           <div class="sub_btn">
             <button type="button" class="btn btn-secondary cancel_btn" data-dismiss="modal">{{ __('Cancel')}}</button>
             <button type="button" id="checkInModalBtn" class="btn btn-primary">{{ __('Check in')}}</button>
           </div>
         </div>
       </div>
     </form>
...
<div class="modal fade" id="checkInSummeryModal" tabindex="-1" role="dialog" aria-labelledby="checkInSummModalTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable cust_sco" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="checkInSummModalTitle">{{ __('Check In Summery')}}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<div class="modal fade" id="pickDineModal" tabindex="-1" role="dialog" aria-labelledby="checkInSummModalTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-scrollable cust_sco" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="checkInSummModalTitle">{{ __('Pick or Dine')}}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
          ...
          $(document).ready(function(){
            ...
            $(document).ready(function() {
              ...
              submitHandler: function(form) {
                ...
                var tableData = '';
                $.post("{{route('register.visit.storeAjax')}}",formData, function(data, status){
                  if(status == 'success'){
...
<script>
  ...
  function sendNewMessage() {
    ...
    var data = {'sendTo':sendTo,'chatId':myStorage.chatID,'restorant_id':'{{$restorant->id}}','msg':newMessage}
    var messagesContainer = $('.messages');
    $.post("{{route('send-message')}}",data,function(resp,status){
      if(resp.status){
  ...
  function getMessages(messagesContainer) {
    var data = {'chatId':myStorage.chatID,'restorant_id':'{{$restorant->id}}'};
     Interval = window.setInterval(function(){
      $.get("{{route('get-message')}}",data,function(resp,status){
