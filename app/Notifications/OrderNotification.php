    ...
    public function toMail($notifiable)
    {
        ...
        //Add order details
            ...
            foreach ($this->order->items as $key => $item) {
                $lineprice= $item->pivot->qty." X ".$item->name." ( ".money( $item->price, env('CASHIER_CURRENCY','usd'),env('DO_CONVERTION',true))." ) = ".money( $item->pivot->qty*$item->price, env('CASHIER_CURRENCY','usd'),env('DO_CONVERTION',true));
                $message->line($lineprice);
            }
            $message->line(__('________________'));

            if($this->order->delivery_method && $this->order->delivery_method.""=="1"){
                $message->line(__('Delivery').": ".money( $this->order->delivery_price, env('CASHIER_CURRENCY','usd'),env('DO_CONVERTION',true)));
            }

            $message->line(__('Total').": ".money( $this->order->order_price, env('CASHIER_CURRENCY','usd'),env('DO_CONVERTION',true)));
